...
def _ising_mels_jax(x, edges, h, J, diag_shift):
    if h == 0:
        max_conn_size = 1
    else:
    ...
    mels = mels.at[0].set(diag_shift + J * (2 * same_spins - 1).sum())
    if h != 0:
...
def _ising_kernel_jax(x, edges, flip, h, J, local_states, diag_shift):
    batch_shape = x.shape[:-1]
    x = x.reshape((-1, x.shape[-1]))

    mels = _ising_mels_jax(x, edges, h, J, diag_shift)
...
class Ising(SpecialHamiltonian):
    ...
    def __init__(
        ...
        self._diag_shift = np.array(constant, dtype=dtype)
        ...
        self._diag_shift_jax = jnp.array(self._diag_shift)
    ...
    @staticmethod
    @jit(nopython=True)
    def _flattened_kernel(x, sections, edges, h, J, diag_cnst):  # pragma: no cover
        n_sites = x.shape[1]
        n_conn = n_sites + 1
    ...
    def _get_conn_flattened_closure(self):
        ...
        _h = self._h
        _J = self._J
        _diag_shift = self._diag_shift
        fun = self._flattened_kernel

        def gccf_fun(x, sections):  # pragma: no cover
            return fun(x, sections, _edges, _h, _J, _diag_shift)
    ...
    def get_conn_padded(self, x):
        return _ising_kernel_jax(
            ...
            self._diag_shift_jax,
        )

    def __repr__(self):
        return f"Ising(J={self._J}, h={self._h}; dim={self.hilbert.size}, shift={self._diag_shift})"
    ...
    def __add__(self, other):
        if (type(self) is type(other)) or is_scalar(other):
            res = self.copy()
            res += other
            return res
    ...
    def __iadd__(self, other):
        if type(self) is type(other):
            ...
            self._diag_shift = np.array(other, dtype=self.dtype)
            self._diag_shift_jax = jnp.array(self._diag_shift)
            return self
    ...
    def __imul__(self, other):
        if is_scalar(other):
            self._h = self._h * other
            self._J = self._J * other
            self._diag_shift = self._diag_shift * other
            self._diag_shift_jax = self._diag_shift_jax * other
            return self
        return NotImplemented
