...
from netket_fidelity.examples.twospins_GHZ.GHZ_state import GHZ
from netket_fidelity.examples.twospins_GHZ.RBM_Jastrow_ansatz import RBM_Jastrow
...
te = nkf.driver.infidelity_optimizer.InfidelityOptimizer(GHZ_vstate, optimizer, variational_state=phi, cv_coeff=-0.5)
...
def Trotter_Ising(phi, optimizer, psi, Uxs, Uxs_dagger, J, ts, n_iter, obs=None): 
    ...
    for t in ts: 
        ...
        for (l,m) in g.edges():
            params["theta_zz"] = params["theta_zz"].at[l, m].set(params["theta_zz"].at[l, m].get() - J * dt / 2)
    ...
    if(obs is not None): 
        ...
        return psi
# Run the evolution 
psi, obs_dict = Trotter_Ising(phi, optimizer, psi, Uxs, Uxs_dagger, J, ts, n_iter=n_iter, obs=obs)
